 ...
 * Slot assignment: slot = address_hash[0] (first byte).
 *
...
int moor_dht_handle_pir_query(moor_circuit_t *circ,
     ...
     * address_hash[0] == i.  When multiple entries collide on the same slot,
    ...
    for (uint32_t i = 0; i < g_dht_store.num_entries; i++) {
        uint8_t slot = g_dht_store.entries[i].address_hash[0];
...
/* Helper: receive a PIR response (1024 bytes, multi-cell) from a connection.
 * PIR_RESPONSE wire: query_id(4) + total_len(2 BE) + offset(2 BE) + data
...
static int dht_recv_pir_response(moor_connection_t *conn, moor_circuit_t *circ,
    ...
    for (int cells = 0; cells < 8; cells++) { /* safety limit */
        moor_cell_t rc;
        ...
        if (rp.relay_command != RELAY_DHT_PIR_RESPONSE) {
            LOG_WARN("DHT PIR: unexpected relay cmd %d", rp.relay_command);
            return -1;
        }
        if (rp.data_length < 8) {
...
static int dht_fetch_pir(const uint8_t address_hash[32],
    ...
    /* Slot index = first byte of address_hash */
    uint8_t slot = address_hash[0];
...
int moor_dht_fetch(const uint8_t address_hash[32],
    ...
    /* Try PIR path first if enabled and we have at least 2 replicas */
    {
        extern moor_config_t g_config;
        if (g_config.pir && resp.num_relays >= 2) {
            LOG_INFO("DHT fetch: attempting PIR fetch");
